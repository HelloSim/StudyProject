package com.sim.basicres.views.shadow;
...
public class ShadowLayout extends FrameLayout {
    ...
    private void initAttributes(AttributeSet attrs) {
        TypedArray attr = getContext().obtainStyledAttributes(attrs, R.styleable.common_ShadowLayout);
        if (attr == null) {
            return;
        }
        ...
        try {
            //默认是显示
            isShowShadow = !attr.getBoolean(R.styleable.common_ShadowLayout_common_hl_shadowHidden, false);
            leftShow = !attr.getBoolean(R.styleable.common_ShadowLayout_common_hl_shadowHiddenLeft, false);
            rightShow = !attr.getBoolean(R.styleable.common_ShadowLayout_common_hl_shadowHiddenRight, false);
            bottomShow = !attr.getBoolean(R.styleable.common_ShadowLayout_common_hl_shadowHiddenBottom, false);
            topShow = !attr.getBoolean(R.styleable.common_ShadowLayout_common_hl_shadowHiddenTop, false);
            mCornerRadius = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_cornerRadius, 0);
            mCornerRadius_leftTop = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_cornerRadius_leftTop, -1);
            mCornerRadius_leftBottom = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_cornerRadius_leftBottom, -1);
            mCornerRadius_rightTop = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_cornerRadius_rightTop, -1);
            mCornerRadius_rightBottom = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_cornerRadius_rightBottom, -1);

            //默认扩散区域宽度
            mShadowLimit = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_shadowLimit, 0);
            if (mShadowLimit == 0) {
            ...
            //x轴偏移量
            mDx = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_shadowOffsetX, 0);
            //y轴偏移量
            mDy = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_shadowOffsetY, 0);
            mShadowColor = attr.getColor(R.styleable.common_ShadowLayout_common_hl_shadowColor, getResources().getColor(R.color.common_translucent));

            selectorType = attr.getInt(R.styleable.common_ShadowLayout_common_hl_shapeMode, 1);
            isSym = attr.getBoolean(R.styleable.common_ShadowLayout_common_hl_shadowSymmetry, true);

            //背景颜色的点击(默认颜色为白色)
            mBackGroundColor = getResources().getColor(R.color.common_white);

            Drawable background = attr.getDrawable(R.styleable.common_ShadowLayout_common_hl_layoutBackground);
            if (background != null) {
            ...
            Drawable trueBackground = attr.getDrawable(R.styleable.common_ShadowLayout_common_hl_layoutBackground_true);
            ...
            //边框颜色的点击
            stroke_color = attr.getColor(R.styleable.common_ShadowLayout_common_hl_strokeColor, -101);
            stroke_color_true = attr.getColor(R.styleable.common_ShadowLayout_common_hl_strokeColor_true, -101);

            if (stroke_color == -101 && stroke_color_true != -101) {
                throw new UnsupportedOperationException("使用了ShadowLayout_hl_strokeColor_true属性，必须先设置ShadowLayout_hl_strokeColor属性");
            }

            stroke_with = attr.getDimension(R.styleable.common_ShadowLayout_common_hl_strokeWith, dip2px(1));
            //规定边框长度最大不错过7dp
            ...
            Drawable clickAbleFalseBackground = attr.getDrawable(R.styleable.common_ShadowLayout_common_hl_layoutBackground_clickFalse);
            ...
            isClickable = attr.getBoolean(R.styleable.common_ShadowLayout_common_clickable, true);
