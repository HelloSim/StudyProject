...
import com.sim.traveltool.adapter.bus.StationListAdapter;
...
public class BusRealTimeActivity extends BaseActivity {
    ...
    private void getStationList(String lineId) {
        stationList.clear();
        retrofitUtil2.getStationList(new Subscriber<BusRealTimeByLineDataBean>() {
            @Override
    ...
    private void getBusListOnRoad(String lineName, String fromStation) {
        ...
        retrofitUtil2.getBusListOnRoad(new Subscriber<BusRealTimeDataBean>() {
            @Override
            public void onCompleted() {
                ...
                if (stationListAdapter == null) {
                    stationListAdapter = new StationListAdapter(context, stationList, busListOnRoadListList);
                    stationListAdapter.setHasStableIds(true);
                    rlstationList.setLayoutManager(new LinearLayoutManager(context));
